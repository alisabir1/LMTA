---
title: "Forecast of Labour Market Transfer Agreements (LMTA) Client Numbers and Expenditures FY 2024-25 to FY 2028-29"
...
The purpose of this report is to undertake an analysis of past and present LMTA-claimed programs, to forecast LMTA client numbers and expenditures for the next five years. The forecast is important considering recent amendments to the Employment Insurance (EI) act under Bill C-19. The amendments include expanding the definition of an insured participant to include more workers, and to remove the requirement that a person be unemployed. This is expected to increase LMTA client numbers and expenditures going forward. The information in this report will be used to inform discussion between the Ministry of Labour, Immigration, Training and Skills Development (MLITSD) and the Federal Government regarding expected LMTA client numbers and expenditures.


# Data

Data used for the purpose of this report includes both MLITSD's internal and external resources.
...
lfs_emp<-
lfs_er_raw %>%
  filter(statistics=="Estimate", geo=="Ontario",sex=="Both sexes",labour_force_characteristics=="Employment", age_group=="15 years and over", data_type=="Seasonally adjusted")%>%  
...
```{r echo=FALSE}
lfs_emp %>% plot_time_series(date, value)
```


```{r include=FALSE}
emp_split<-
time_series_split(lfs_emp,
                  assess="6 years",
                  cumulative=T)
...
arima_model <-arima_reg() %>% 
  ...
  fit(value~date,training(emp_split))
...
calib_tbl<-model_tbl %>% 
  modeltime_calibrate(testing(emp_split))
...
```
    ...
    * The table below represents the actual and predicted Employment in Ontario between FY 2018-19 and FY 2028-29.For simplicity of notation, FY 2018-19 has been termed as "2019", FY 2019-20 as “2020", and so on. Employment corresponding to each fiscal year is the average of employment of all the months in that FY.
...
datatable(
  emp_forecast,
  caption= "Table 1: Actual and Predicted Mean Employment in Ontario; FY 2018-19 till FY 2028-29",
colnames = c("Year", "Employment"),
rownames = FALSE,
...
Sum_EI<-
EI_Status_2019_23 %>% 
filter(program=="Grand Total", year!=2019,year!=2020
) %>% 
  ungroup() %>% 
  select(year, total_by_program)
...
join_EI_emp<-
inner_join(emp_forecast,Sum_EI, by="year")
...
The following analysis will test for relationship between employment and number of EI clients. If a relationship is found, number of EI clients will be predicted for the next five years using employment forecasts.
...
```{r echo=FALSE}
cor(join_EI_emp$emp, join_EI_emp$total_by_program)
```


```{r eval=FALSE, include=FALSE}
cor.test(join_EI_emp$emp, join_EI_emp$total_by_program)
...
join_EI_emp %>% 
  ggplot(aes(emp, total_by_program))+
  geom_line(alpha=0.8, size=1.2, color=cl_blue)+
  geom_smooth(method=lm, se=F, colour = "darkgray", size = 1)+
  labs(title= "Figure 2: EI Clients by Employment in Ontario",
         x="Employment in Ontario",
...
Therefore, we can fit a linear model between the two variables for the purpose of forecasting. 

Table 3 below shows forecasts for 'Employment' as well as 'EI Client Forecast', which is forecast for the sum of EI eligible and non-EI eligible clients. 'Year' 2024 shows forecast for all of FY 2023-24 i.e. April 1 2023 to March 31 2024. Forecasts for the following five years till FY 2028-29 are also shown.


```{r include=FALSE}
model_EI<-
lm(total_by_program~emp, data = join_EI_emp)
...
y <- predict.lm(model_EI, data.frame(emp=emp_forecast_test))  
...
colnames = c("Year", "Employment", "EI Client Forecast"),
rownames = FALSE,
    options = list(info = FALSE,
    paging = FALSE,
    searching = FALSE,
...
join_LMTA_emp<-
inner_join(emp_forecast,LMTA_claims, by="year")
...
```{r echo=FALSE}
cor(join_LMTA_emp$emp, join_LMTA_emp$mean)

```

```{r eval=FALSE, include=FALSE}
cor.test(join_LMTA_emp$emp, join_LMTA_emp$mean)
...
```{r eval=FALSE, include=FALSE}
plot(join_LMTA_emp$emp, join_LMTA_emp$mean, main="Correlation between Unemployment and LMTA clients", xlab="Unemployment Number", ylab="number of EI clients")
...
join_LMTA_emp %>% 
  ...
  geom_line(alpha=0.8, size=1.2, color=cl_blue)+
  geom_smooth(method=lm, se=F, colour = "darkgray", size = 1)+
  labs(title= "Figure 4: LMTA Claim Numbers by Employment in Ontario",
         x="Employment in Ontario",
...
Table 5 below shows forecasts for 'Employment' as well as 'LMTA Client Forecast'. 

```{r include=FALSE}
model<-
lm(mean~emp, data = join_LMTA_emp)
...
y <- predict.lm(model, data.frame(emp=emp_forecast_test))  
...
colnames = c("Year", "Employment", "LMTA Claim Forecast"),
rownames = FALSE,
    options = list(info = FALSE,
    paging = FALSE,
    searching = FALSE,
...
*Despite decrease in LMTA claims during COVID, the overall trend has been for claims to increase from FY 2018-19 till FY 2022-23 (Figure 3). This trend will continue to increase as shown in Table 5 that LMTA claims are expected to continue increasing till FY 2028-29.*



## Claim Expenditure

Figure 5 and Table 6 below show LMTA claim expenditures for each of the years from FY 2018-19 till FY 2023-24.

Figure 5(a) on the left shows an increasing trend in claim expenditure for the full period. Claim expenditure during FY 2020-21 and FY 2021-22 is higher than the overall trend. This increase in claim expenditure, despite the fall in claim numbers around the same time shown in Figure 4, could have been a result of more intensive and longer duration of employment services required for clients during the pandemic.

Figure 5(b) on the right shows the trend when excluding FY 2020-21 and FY 2021-22. Even removing the two outliers, the trend is still increasing at the same rate as the full period. 
...
Table 7 below represents LMTA claim expenditure projections obtained using Holt's Trend Method. This method is used to forecast future values based on the existing trend in LMTA claims expenditure. The 'Forecast' for LMTA expenditure is given alongside the lower and upper bounds based on the confidence interval.
...
*Trend from FY 2018-19 till FY 2023-24 shows an increasing trend in LMTA claim expenditure. With GDP growth expected to remain subdued in the near term, forecast for LMTA claim expenditures is to increase till the end of FY 2028-29 (Table 7) *
...
geom_bar(stat = "identity", fill=cl_maroon)+
  ...
  geom_smooth(aes(fill=NULL),  method=lm, se=FALSE, col=cl_blue)+
    labs(title= "(a) EI Eligible Client \n Numbers",
         subtitle = "2019-2023",
           y="Client Numbers",
         x="Year")+
    theme(title = element_text(size=12, colour = cl_gold, face = "bold"))
...
geom_bar(stat = "identity", fill=cl_maroon)+
    scale_y_continuous(labels = label_comma(), limits = c(0,200000))+
  geom_smooth(aes(fill=NULL),  method=lm, se=FALSE, col=cl_blue)+
    labs(title= "(b) EI Eligible Client \n Numbers",
